...
from Application.cluster import (  # <-- import trực tiếp từ file bạn có
    build_cluster_faiss,
    build_clusters,
)
from Application.feature_extract import (  # <-- import trực tiếp từ file bạn có
    mean_extract_image_features,
    mean_extract_image_features_batch_1,
)
...
def evaluate_by_image(base_folder):
    ...
    for group_name in sorted(os.listdir(base_folder)):
        ...
        for cls in all_classes:
            ...
            class_group_candidates[cls] = (
                sorted(candidates) if max_count > 0 else []
            )
        ...
        for group_name, cls_list in list(group_owners.items()):
            ...
            for loser in losers:
                ...
                for g in old_groups:
                    ...
                    max_count_for_loser = max(
                        cls_count.get(loser, 0)
                        for cls_count in group_classes.values()
                    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(IMG_DIR):
        os.makedirs(IMG_DIR)
        print(
            f"Đã tạo thư mục {IMG_DIR}. Vui lòng thêm ảnh vào đó và chạy lại."
        )
    ...
    start = time.time()
    build_cluster_faiss(
        features, filenames, IMG_DIR, "clusters1", threshold=0.8, K=10
    )
    end = time.time()
    ...
    ht = MyHash.HashTable(36, features.shape[1])
    ht1 = MyHash.BloomFilter(108, features.shape[1], 9)
    ht2 = MyHash.SimHash(73)
    ht3 = MyHash.MinHash(64)
    ...
    start = time.time()
    bloomfilter = build_clusters(
        ht1, features, filenames, IMG_DIR, 21, "clusters3"
    )
    end = time.time()
    ...
    start = time.time()
    minhash = build_clusters(
        ht3, features, filenames, IMG_DIR, 316, "clusters5"
    )
    end = time.time()
